...
def compute_r2_metrics(y_true, y_pred, group_labels=None):
    ...
    metrics['r2_diag'] = [r2_score(y_true[:, i], y_pred[:, i]) for i in range(y_true.shape[1])]

    metrics['r2_sep'] = {}
    if group_labels is not None:
        group_labels = np.array(group_labels)
        for group in np.unique(group_labels):
            idxs = group_labels == group
            metrics['r2_sep'][str(group)] = r2_score(y_true[idxs], y_pred[idxs])
    return metrics
...
def train_model(model_class, train_loader, val_loader, 
    ...
    if test_loader is not None:
        ...
        for file_path, prefix in model_paths:
            ...
            print(f"isCausalWorld: {isCausalWorld}")
            '''
            # === Additional R² Evaluation for CausalWorldDataset ===
            if isCausalWorld:
                model.eval()
                ...
                with torch.no_grad():
                    for batch in test_loader:
                        ...
                        #x = batch['obs'].to(model.device)
                        #y = batch['target'].to(model.device)
                        pred = model(x)
                ...
                group_labels = all_groups if len(all_groups) > 0 else None

                r2_metrics = compute_r2_metrics(y_true, y_pred, group_labels)

                print('\n✅ R² Metrics:')
                print(f'R² (global): {r2_metrics["r2_global"]:.4f}')
                print(f'R² (diag): {np.round(r2_metrics["r2_diag"], 4)}')
                if r2_metrics['r2_sep']:
                    print('R² (sep):')
                    for k, v in r2_metrics['r2_sep'].items():
                        print(f'  {k}: {v:.4f}')

                # Add to test_result to save in JSON
                test_result['r2_global'] = r2_metrics["r2_global"]
                test_result['r2_diag'] = r2_metrics["r2_diag"]
                test_result['r2_sep'] = r2_metrics["r2_sep"]
                '''
            print('='*50)
